...
f_plot(a,b, t0,t) = (t < x_min || t > x_max)? NaN : a*log((t-t0)) + b
	...
	#------------------------------------ DATA, FIT AND PLOT
	name(x) = 0; key_title(x) = 0; rescale_X(x,y) = 0;
	name2(x) = 0;
	i0 = 0; iend = 0; di = 1;
		if(scaling == 0){
	...
	exit;
	RANGE="set xrange[0.2:180]; set yrange[0.1:6];"
	...
	if(what_to_plot==1){
		if(compare_scales){
			...
			set multiplot
			@MARGIN; @RANGE;
			plot for[i=i0:iend:di] name(i) u (rescale_X($1,i)):(rescale($2,i)) w lp ps 0.75 pt 5 title key_title(i),\
