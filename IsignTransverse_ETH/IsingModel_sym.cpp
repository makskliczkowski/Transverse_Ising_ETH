...
/// <returns> index of the representative state in the EC </returns>
 std::tuple<u64, int> IsingModel_sym::find_translation_representative(u64 base_idx) const {
	...
	while (idx != base_idx) {
		idx = rotate_left(base_rotate, this->BinaryPowers[L - 1]); // new rotate by decimal representation
		base_rotate = idx;
		counter++;
		if (idx < EC_sym) {
			EC_sym = idx;
...
/// <returns></returns>
 std::tuple<u64, int> IsingModel_sym::find_SEC_representative(u64 base_idx) const {
	...
	u64 idx_R = 0;
	if (this->k_sector) {
		idx_R = reverseBits(base_idx, this->L, this->BinaryPowers);
	...
	if (this->h == 0) {
		...
		// check spin-flip
		auto tupleX = find_translation_representative(flip(base_idx, this->BinaryPowers[L]));
		minima.push_back(std::get<0>(tupleX));

		// check spin-flip and reflection
		int eig_RXsym = INT_MAX;
		if (this->k_sector) {
			auto tupleRX = find_translation_representative(flip(idx_R, this->BinaryPowers[L]));
...
/// <returns></returns>
 cpx IsingModel_sym::get_symmetry_normalization(u64 base_idx) {
	...
	u64 PT = (k_sector) ? reverseBits(base_idx, this->L, this->BinaryPowers) : INT_MAX;
	u64 ZT = flip(base_idx, this->BinaryPowers[L]);
	u64 PZT = (k_sector) ? flip(PT, this->BinaryPowers[L]) : INT_MAX;

	for (int l = 0; l < this->L; l++) {
		if (Translation == base_idx)
			normalisation += this->k_exponents[l];
		Translation = rotate_left(Translation, this->BinaryPowers[L - 1]);
		if (this->k_sector){
			if (PT == base_idx) 
				normalisation += ((double)this->symmetries.p_sym) * this->k_exponents[l];
			PT = rotate_left(PT, this->BinaryPowers[L - 1]);
		}
		if ((this->h == 0)){
			if (ZT == base_idx)
				normalisation += ((double)this->symmetries.x_sym) * this->k_exponents[l];
			ZT = rotate_left(ZT, this->BinaryPowers[L - 1]);
			if (this->k_sector) {
				if (PZT == base_idx)
					normalisation += ((double)this->symmetries.p_sym * (double)this->symmetries.x_sym) * this->k_exponents[l];
				PZT = rotate_left(PZT, this->BinaryPowers[L - 1]);
			}
		}
	}
...
/// </summary>
 void IsingModel_sym::generate_mapping() {
	 ...
	 u64 two_powL = this->BinaryPowers[L];
	if (num_of_threads == 1)
	...
	for (long int k = 0; k < this->N; k++) {
		...
		for (int j = 0; j <= this->L - 1; j++) {
			...
			if (this->g != 0) {
				u64 new_idx = INT_MAX;
				if (base_vector[j])
					new_idx = u64((int64_t)mapping[k] - (int64_t)this->BinaryPowers[L - 1 - j]);
				else
					new_idx = (int64_t)mapping[k] + (int64_t)this->BinaryPowers[L - 1 - j];
...
{
	const u64 base_idx = binary_search(sec_beta.mapping, 0, sec_beta.N - 1, binary_to_int(base_vec));							// index of the current base vector
	...
	auto get_overlap_sym = [&](const v_1d<bool>& vec_sym, cpx sym_eig) {
		...
		if (abs(val_sym_beta) > 1e-14) {
			auto [idx_sym, val_sym_alfa] = !(vec_sym == vec_sym_tmp) ?\
				find_rep_and_sym_eigval(binary_to_int(vec_sym_tmp), sec_alfa, sec_beta.normalisation[base_idx]) :\
				std::make_pair(binary_to_int(vec_sym), conj(sym_eig));
			if (idx_sym < sec_alfa.N)
				overlap = sym_eig * conj(val_sym_alfa * alfa(idx_sym)) * beta(base_idx) * val_sym_beta;
		}
		return overlap;
	};
