...
# -no-multibyte-chars -pthread -lstdc++fs -qopenmp -qmkl=parallel -std=c++17 -O3 >& compile_benchmark.log

clang++ main.cpp IsingModel.cpp IsingModel_disorder.cpp IsingModel_sym.cpp tools.cpp user_interface.cpp -o Ising_bench.o\
 -llapack -lblas -lopenblas\
 -lrt -pthread -lstdc++fs -fopenmp -fomit-frame-pointer -std=c++17 -O3 >& compile_benchmark.log

now="$(date +'date=%d_%m_time=%H_%M')"
./Ising_bench.o -L 15 -Ln 8 -Ls 1 -g 0.7 -h 0.8 -w 0.01 -th 64 -m 0 -w 0.01 -r 1 -op 0 -fun 5 -s 1 -b 0 >& benchmark_${now}.txt
