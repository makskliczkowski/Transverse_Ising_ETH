...
sbatch --job-name=$name --output=logs/${name}_log-%A-%a.out --cpus-per-task=8 --mem=4G --time=24:00:00 --array=0-23%12 Ising_g_scaling.sh 11 $h 0.05 $fun $operator $site 8 "${@:6}"
sbatch --job-name=$name --output=logs/${name}_log-%A-%a.out --cpus-per-task=16 --mem=6G --time=24:00:00 --array=0-23%12 Ising_g_scaling.sh 12 $h 0.05 $fun $operator $site 16 "${@:6}"
sbatch --job-name=$name --output=logs/${name}_log-%A-%a.out --cpus-per-task=16 --mem=8G --time=96:00:00 --array=0-23%12 Ising_g_scaling.sh 13 $h 0.05 $fun $operator $site 16 "${@:6}"
sbatch --job-name=$name --output=logs/${name}_log-%A-%a.out --cpus-per-task=16 --mem=12G --time=168:00:00 --array=0-23%12 Ising_g_scaling.sh 14 $h 0.05 $fun $operator $site 16 "${@:6}"
