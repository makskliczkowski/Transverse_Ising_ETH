...
sbatch --job-name="${name}_L=12" --output=logs/${name}_log-%A-%a.out --cpus-per-task=2 --mem=4G --time=24:00:00 --array=0-9%5 Ising_parameter_scaling.sh 12 $x $y 0.05 $fun $operator $site 2 $jobid "${@:8}"
#sbatch --job-name="${name}_L=13" --output=logs/${name}_log-%A-%a.out --cpus-per-task=2 --mem=8G --time=72:00:00 --array=0-9%5 Ising_parameter_scaling.sh 13 $x $y 0.05 $fun $operator $site 2 $jobid "${@:8}"
#sbatch --job-name="${name}_L=14" --output=logs/${name}_log-%A-%a.out --cpus-per-task=2 --mem=16G --time=72:00:00 --array=0-19%4 Ising_parameter_scaling.sh 14 $x $y 0.05 $fun $operator $site 4 $jobid "${@:8}"
