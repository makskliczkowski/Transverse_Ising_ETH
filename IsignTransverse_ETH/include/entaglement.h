...
namespace entropy{
    ...
    double vonNeumann(
    	...
    	auto rho = entaglement::reduced_density_matrix(state, A_size, int L);
    ...
    double reyni_entropy(
	    ...
	    auto rho = entaglement::reduced_density_matrix(state, A_size, L);
	    return log2(real(trace(arma::powmat(rho, alfa)))) / (1.0 - alfa);
    }
