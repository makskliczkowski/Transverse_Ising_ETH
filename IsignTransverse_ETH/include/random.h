...
public:
	...
	template <typename _type> 
	arma::Col<_type> create_random_vec(const uint64_t size) {
		arma::Col<_type> random_vec(size, arma::fill::zeros);
		for (uint64_t j = 0; j < size; j++) {
			random_vec(j) = random_uni<_type>(-0.5, 0.5);
		}
		return arma::normalise(random_vec);
	}
	template <typename _type> 
	std::vector<_type> create_random_stdvec(const uint64_t size) {
		std::vector<_type> random_vec(size, 0);
		_type norm = 0;
		for (uint64_t j = 0; j < size; j++) {
			random_vec[j] = random_uni<_type>(-1, 1);
			norm += random_vec[j];
		}
		for (uint64_t j = 0; j < size; j++)
			random_vec[j] /= norm;
