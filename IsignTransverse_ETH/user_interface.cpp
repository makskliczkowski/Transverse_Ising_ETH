	...
	*/
	...
	for (double w = 0.0; w <= 5.0; w += 0.2)
	{
		...
		for (int k = 0; k < realisations - 1; k++)
		{
			...
			if (k % 5 == 0)
				stout << " \t\t--> " << k << " realisation - in time : " << tim_s(start) << "s" << std::endl;
		}
...
void isingUI::ui::size_scaling_sym(int k, int p, int x)
{
	...
	for (int Lx = this->L; Lx <= L_max; Lx += this->Ls)
	{
		...
		const long int E_min = alfa->E_av_idx - long(mu / 2);
		const long int E_max = alfa->E_av_idx + long(mu / 2);
		stout << " \t\t--> finished diagonalizing for " << alfa->get_info() << " - in time : " << double(duration_cast<milliseconds>(duration(system_clock::now() - start)).count()) / 1000.0 << "s" << std::endl;
...
void isingUI::ui::parameter_sweep_sym(int k, int p, int x)
{
	...
	for (double gx = 0.8; gx < gmax; gx += this->gs)
	{
		...
		for (auto &hx : arma::logspace(-4, 1, 100))
		{
			...
			farante << gx << "\t\t" << hx << "\t\t" << ipr / double(counter * N) << "\t\t" << ent / double(counter) << "\t\t" << r / double(counter) << "\t\t" << exp(typ_susc / double(counter)) << endl;
										   ...
										   double energy_constraint, int energy_num, std::initializer_list<int> alfa_sym, std::initializer_list<int> beta_sym) const
{
	...
	for (int a = alfa->E_av_idx - energy_num / 2; a < alfa_max_E; a++)
	{
		...
		for (int b = beta->E_av_idx - energy_num / 2; b < beta_max_E; b++)
		{
			...
			if (abs((Eb + Ea) / (2.0 * E_av) - 1.0) > energy_constraint / 2.0)
				continue;
	...
	d_sigma_x = normalise_dist(d_sigma_x, min, max);
	d_sigma_x_ex = normalise_dist(d_sigma_x_ex, min, max);
	d_sigma_z_nnn = normalise_dist(d_sigma_z_nnn, min, max);
	d_sigma_z_nnn_ex = normalise_dist(d_sigma_z_nnn_ex, min, max);
	// d_sigma_flip = normalise_dist(d_sigma_flip, min, max);
	for (int i = 0; i < size; i++)
		op_prob_dist << i * step + min << "\t\t" << d_sigma_x(i) << "\t\t" << d_sigma_x_ex(i) << "\t\t" << d_sigma_z_nnn(i) << "\t\t" << d_sigma_z_nnn_ex(i) << "\t\t\n"; // << d_sigma_flip(i) << "\n";
	for (int i = 0; i < omega_num; i++)
	{
		gaussianity << i * omega_step << "\t\t" << (av_sigma_x2[i] / av_sigma_x[i]) / av_sigma_x[i] * counter[i] << "\t\t" << (av_sigma_x2_ex[i] / av_sigma_x_ex[i]) * av_sigma_x_ex[i] * counter[i] << std::endl;
	}
	gaussianity.close();
	op_prob_dist.close();
}
...
std::vector<double> isingUI::ui::perturbative_stat_sym(double dist_step, double min, double max, double pert, IsingModel_sym &alfa, IsingModel_sym &beta)
{
	...
	for (int i = 0; i < size; i++)
	{
		...
		if (i < E_size)
			dis_E << E_dist_step * i + min << "\t\t" << dis_delta_E(i) << "\t\t" << gaussian(E_dist_step * i + min, 0.0, arma::stddev(dis_sig_x)) << "\n";
	}
...
void isingUI::ui::spectralFunction(IsingModel<_type> &alfa, const arma::cx_mat &mat_elem, std::string name)
{
	const u64 N = alfa.get_hilbert_size();
...
template <typename _type>
void isingUI::ui::integratedSpectralFunction(const IsingModel<_type> &alfa, const arma::cx_mat &mat_elem, std::string name)
{
	const u64 N = alfa.get_hilbert_size();
...
template <typename _type>
auto isingUI::ui::integratedSpectralFunction(const IsingModel<_type> &alfa, const arma::cx_mat &mat_elem, const arma::vec &omegas) -> arma::vec
{
	const u64 N = alfa.get_hilbert_size();
...
template <typename _type>
void isingUI::ui::timeEvolution(const IsingModel<_type> &alfa, const arma::cx_mat &mat_elem, std::string name)
{
	const u64 N = alfa.get_hilbert_size();
...
template <typename _type>
auto isingUI::ui::timeEvolution(const IsingModel<_type> &alfa, const arma::cx_mat &mat_elem, const arma::vec &times) -> std::pair<arma::vec, double>
{
	const u64 N = alfa.get_hilbert_size();
	const double tH = 1. / alfa.mean_level_spacing_analytical();
	double LTA = 0;
...
void isingUI::ui::intSpecFun_from_timeEvol()
{
	...
	for (auto &gx : gx_list)
	{
		for (auto &hx : hx_list)
		{
			...
			std::string filename = this->saving_dir + "timeEvolution" + kPSep + "j=0" + kPSep + name;
			auto data = readFromFile(file, filename);
			if (data.empty())
				continue;
			file.close();
			...
			std::string dir = this->saving_dir + "IntegratedResponseFunction" + kPSep + "integrated" + kPSep + "j=0" + kPSep;
			createDirs(dir);
...
template <typename _type>
std::pair<double, double> isingUI::ui::operator_norm(arma::sp_cx_mat &opMatrix, IsingModel<_type> &alfa)
{
	const u64 N = alfa.get_hilbert_size();
...
template <typename _type>
void isingUI::ui::energyEvolution(IsingModel<_type> &alfa)
{
...
template <typename _type>
void isingUI::ui::IsingLIOMs(IsingModel<_type> &alfa)
{
	const int L = alfa.L;
	std::ofstream ssf;
	...
	for (int j = 0; j < L; j++)
	{
		...
		if (j == 0)
			timeEvolution(alfa, opMatrix, "SigmaZ_j=" + std::to_string(j));
	}
...
void isingUI::ui::benchmark(bool full)
{
	...
	else
	{
		if (this->m)
		{
			...
			double tim1 = tim_s(start);
			start = std::chrono::system_clock::now();
			alfa->diagonalization(false, "std");
						...
						for (int M = 2; M <= 256; M *= 2)
						{
							...
							std::function to_ave_time = [&]()
							{
								...
								for (int i = 0; i < times.size(); i++)
								{
									...
									auto state2 = lancz.time_evolution_non_stationary(changing_state, t - (i == 0 ? 0.0 : times(i - 1)), M);
							...
							std::ofstream file;
							openFile(file, dir + "TimeEvolution" + alfa->get_info({}) + ",x=" + to_string_prec(x, 4) + ",M=" + std::to_string(M) + ".dat");
